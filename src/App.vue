<script>
import EasySeeSo from 'seeso/easy-seeso';
// import { TrackingState } from 'seeso'

const licenseKey = 'dev_r6lhgvzp6c9qrujmix67580y3r207itavw0fonmf';

export default {
    data() {
        return {
            seeso: null,
            text: ''
        };
    },
    methods: {
        afterInitialized() {
            console.log('sdk init success!')
        },
        afterFailed() {
            console.log('sdk init fail!')
            this.text = '실패'
        },
        onGaze(gazeInfo) {
            console.log('1')
            // if (gazeInfo.trackingState === TrackingState.SUCCESS) {
                let canvas = document.getElementById("output")
                console.log('2')
                canvas.width = window.innerWidth
                console.log('3')
                canvas.height = window.innerHeight
                console.log('4')
                let ctx = canvas.getContext("2d");
                console.log('5')
                ctx.fillStyle = '#FF0000'
                console.log('6')
                ctx.clearRect(0, 0, canvas.width, canvas.height)
                console.log('7')
                ctx.beginPath();
                console.log('8')
                ctx.arc(gazeInfo.x, gazeInfo.y, 10, 0, Math.PI * 2, true);
                console.log('9')
                ctx.fill();
                console.log('0')
            // }
        },
        parseCalibrationDataInQueryString() {
            const href = window.location.href
            const decodedURI = decodeURI(href)
            const queryString = decodedURI.split('?')[1];
            if (!queryString) return undefined
            const jsonString = queryString.slice("calibrationData=".length, queryString.length)
            return jsonString
        },
        onClickCalibrationBtn() {
            const userId = 'hammsik'; // ex) 5e9easf293
            const redirectUrl = 'http://localhost:8083';
            const calibrationPoint = 5;
            EasySeeSo.openCalibrationPage(licenseKey, userId, redirectUrl, calibrationPoint);
        }
    },
    mounted() {
        const renderedText = document.querySelector('.book-text').textContent;
        console.log(renderedText);

        this.seeso = new EasySeeSo();

        // (async () => { this.seeso.init(licenseKey, this.afterInitialized, this.afterFailed) })()
        (async () => { this.seeso.init(licenseKey, this.onGaze, this.afterFailed) })()

        // const calibrationData = this.parseCalibrationDataInQueryString()
        // if (calibrationData) {
        //     (async () => {
        //         this.seeso = new EasySeeSo();
        //         // Don't forget to enter your license key.`
        //         await this.seeso.init(licenseKey, async () => { this.seeso.startTracking(this.onGaze) }, this.afterFailed)
        //     })()
        // } else {
        //     console.log('No calibration data given.')
        //     const calibrationButton = document.getElementById('calibrationButton')
        //     calibrationButton.addEventListener('click', this.onClickCalibrationBtn)
        // }
    },
}
</script>

<template>
    <canvas id="output"></canvas>
    <button id="calibrationButton" style="margin: 0 auto;"> Start Calibration </button>
    <h2 id="title">{{ text }}</h2>
    <h2 id="gazeInfo" style="text-align: center; color: red;"></h2>
    <span class="book-text">한 번에 두둥실한 구름이 뜬 농장 어딘가에, 발이 달린 망고 한 개가 자랐다. 이 망고는 다른 망고들과는 다르게 농장에서 벗어나 세계를 탐험하고 싶었다. 어릴 적부터
        발이 달린 자신의 모습에 기뻐하며 하늘을 바라보곤 했다. 어느 날, 망고는 결심을 하고 농장을 떠났다. 발이 달린 망고는 작은 가방에 몇 가지 필요한 물건을 담아 어디론가 모험을 떠났다. 망고의 여행은
        시작되었다! 첫 번째로 망고는 산을 오르며 높은 곳에서 내려다볼 수 있는 기쁨을 느꼈다. 바람에 흔들리는 나뭇가지 위에서 바라보는 풍경은 너무나 아름다웠다. 발이 달린 망고는 하늘을 향해 높게 높게
        뛰며 자유로움을 느꼈다. 다음으로 망고는 넓은 바다로 향했다. 발로 천천히 걸으면서 파도 소리에 귀를 기울였다. 해변가에서 물결에 발을 담그며 망고는 자연의 아름다움에 감탄했다. 얕은 물에서는 발이
        달린 망고의 특별함이 물결과 함께 춤을 추었다. 여행의 중간에서, 망고는 다양한 사람들을 만났다. 각자의 이야기와 문화를 듣고, 발이 달린 망고는 세계는 얼마나 다양하고 아름다운지를 깨달았다. 망고는
        다양성과 이해를 기르며 새로운 친구들을 만나고 여러 나라의 음식을 맛보았다. 망고의 여행은 어느새 며칠이 지나가면서, 그는 어떤 곳에서든 자신을 편안하게 느꼈다. 발이 달린 망고는 어떤 도전이나
        어려움도 이겨내며 자신의 꿈을 따라가는 용기를 얻었다. 그리고 마침내, 발이 달린 망고는 고향 농장으로 돌아왔다. 여행은 그를 더 강하고 지혜롭게 만들었다. 떠난 동안 쌓인 경험과 이야기들은 망고를
        풍요롭게 키웠다. 농장에 돌
        아온 발이 달린 망고는 다시 함께 자란 친구들과 이야기를 나누며, 언젠가 자신의 이야기를 전하고 다른 망고들을 꿈꾸게 하고 싶다는 마음을 품었다. 발이 달린 망고는 세계
        여행을 통해 얻은 풍부한 경험과 사랑을 나누며, 농장에서의 일상을 새로운 시각에서 감사하게 여기며 살아갔다.</span>
</template>
  